...
func Clean(csvFile string, errorPath string, cleanCSVPath string) (*CleanedData, error) {
	...
	for i, word := range records {
		if isInt(word[0]) && len(word) == 17 {
			...
			rate.HIGH = cleanFloat(strings.TrimSpace(word[1]))
			rate.LOW = cleanFloat(strings.TrimSpace(word[2]))
			rate.CODE = strings.TrimSpace(word[3])
			rate.BUY = cleanFloat(strings.TrimSpace(word[4]))
			rate.SELL = cleanFloat(strings.TrimSpace(word[5]))
			rate.PCP = cleanFloat(strings.TrimSpace(word[6]))
			rate.TCP = cleanFloat(strings.TrimSpace(word[7]))
			rate.VOL = cleanFloat(strings.TrimSpace(word[8]))
			rate.DIVNET = cleanFloat(strings.TrimSpace(word[9]))
			rate.DIVYIELD = cleanFloat(strings.TrimSpace(word[10]))
			rate.EEARNYIELD = cleanFloat(strings.TrimSpace(word[11]))
			rate.PERATIO = cleanFloat(strings.TrimSpace(word[12]))
			rate.PBVRATION = cleanFloat(strings.TrimSpace(word[13]))
			rate.CAP = cleanFloat(strings.TrimSpace(word[14]))
			rate.PROFIT = cleanFloat(strings.TrimSpace(word[15]))
			rate.SHARES = cleanFloat(strings.TrimSpace(word[16]))
...
func cleanFloat(value string) float64 {
